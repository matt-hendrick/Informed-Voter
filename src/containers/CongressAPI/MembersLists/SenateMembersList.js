...
function SenateMembersList() {
  ...
  if (results) {
    representativesDisplay = results.map((data) => {
      ...
      if (data.youtube_account) {
        youtubeLink = 'https://www.youtube.com/' + data.youtube_account;
      ...
      return (
        <Col md="6" lg="4" key={data.id}>
          <Card>
            <Card.Body>
              <Card.Title>
                {data.first_name} {data.last_name} ({data.party}) - Senator of{' '}
                {data.state}
              </Card.Title>
  ...
  return (
    <React.Fragment>
      <Container>
        <Jumbotron>
          <h2>Members of the current U.S. Senate</h2>
        </Jumbotron>
      </Container>
      <Container>
...
export default SenateMembersList;
