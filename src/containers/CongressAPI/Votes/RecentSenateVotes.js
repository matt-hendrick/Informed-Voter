...
function RecentSenateVotes() {
  ...
  if (results) {
    votesDisplays = results.results.votes.map((data) => {
      ...
      let URLDisplay = (
        <Card.Link href={data.url}>Senate.gov Vote Summary</Card.Link>
      );
  ...
  return (
    <React.Fragment>
      <Container>
        <Jumbotron>
          <h2>Recent votes by the U.S. Senate</h2>
        </Jumbotron>
      </Container>
      <Container>
...
export default RecentSenateVotes;
